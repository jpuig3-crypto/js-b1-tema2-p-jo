...
class Triangle {
	...
	isEquilateral() {
		...
		return this.height ===
		(this.base * Math.sqrt(3)) / 2;
	}
}

const parsedTriangles = JSON.parse(myJSON);

const myTriangles = parsedTriangles.map(
...
const newTrianglesJSON = 
	JSON.stringify(equilateralsTriangles);
